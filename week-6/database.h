...
private:
  std::map<Date, std::pair<std::set<std::string>,
...
int Database::RemoveIf(const Functor& functor) {
  ...
  for (auto& date_it : db) {
    auto itr = std::stable_partition(date_it.second.second.begin(),
  ...
  for (auto erase_it = db.cbegin(), next_it = erase_it;
       erase_it != db.cend();
       erase_it = next_it) {
    next_it++;
...
std::vector<std::pair<Date, std::string>>
  Database::FindIf(const Functor& functor) const {
  std::vector<std::pair<Date, std::string>> temp;
  
  for (const auto& date_it : db) {
    auto it = std::find_if(date_it.second.second.begin(),
                           date_it.second.second.end(),
                           [&](const auto& event) -> bool {
      if (functor(date_it.first, *event)) {
